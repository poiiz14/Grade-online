...
(function(){const ok=/^https:\/\/script\.google\.com\/macros\/s\/.+\/exec$/.test(API_BASE);if(!ok){console.error('[CONFIG] API_BASE ไม่ถูกต้อง:',API_BASE);alert('API_BASE ยังไม่ใช่ URL /exec ของ Apps Script');}})();
...
function callAPI(action, data={}, {timeoutMs=30000,retries=1,backoffMs=800}={}) {
  function once(t){
    return new Promise((resolve,reject)=>{
      ...
      s.onerror=(e)=>{clearTimeout(timer);cleanup();reject(new Error(`API network error: ${action}`))}
      s.src=url;document.body.appendChild(s);
    });
  }
  return new Promise(async (res,rej)=>{let n=0,last;while(n<=retries){try{res(await once(timeoutMs));return;}catch(e){last=e;n++;if(n>retries)break;await new Promise(r=>setTimeout(r,backoffMs*Math.pow(2,n-1)));} }rej(last);});
}
...
async function handleLoginSubmit(ev){
  ...
  }catch(e){Swal.close();Swal?.fire({icon:'error',title:'เกิดข้อผิดพลาด',text:String(e?.message||e)});}
}
...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  const btn=document.querySelector('button[type="submit"]');btn?.addEventListener('click',e=>{e.preventDefault();handleLoginSubmit(e);});
});

/* ===================== ADMIN ===================== */
function showOnlyDashboard(id){['adminDashboard','studentDashboard','advisorDashboard'].forEach(x=>document.getElementById(x)?.classList.add('hidden'));document.getElementById(id)?.classList.remove('hidden');}
...
function showStudentDashboard(){
  ...
  const meLatest=latest[me.id]; setText('studentEnglishStatus', meLatest?.status || '-');
  ...
  window.showSemester=function(sem){ document.querySelectorAll('.semester-tab').forEach(b=>b.classList.remove('border-blue-500','text-blue-600')); 
    const idx={'1':0,'2':1,'3':2}[sem]||0; document.querySelectorAll('.semester-tab')[idx].classList.add('border-blue-500','text-blue-600'); 
  ...
  function updateStudentSemester(sem){
    const semester=sem || (document.querySelector('.semester-tab.border-blue-500')?.textContent.includes('2')?'2':document.querySelector('.semester-tab.border-blue-500')?.textContent.includes('ฤดูร้อน')?'3':'1');
...
function showTeacherDashboard(){
  ...
  setText('advY1', y1); setText('advY2', y234);

  const latest=latestEnglishMap(GLOBAL_DATA.englishTests.filter(e=>advisees.some(s=>String(s.id)===String(e.studentId))));
  let pass=0; Object.values(latest).forEach(r=>{const s=(r.status||'').toString().toLowerCase(); if(['ผ่าน','pass','passed','p'].includes(s)) pass++;});
  ...
  advisees.forEach(s=>{
    ...
    div.innerHTML=`<div class="flex justify-between">
      ...
      <button class="text-blue-600 hover:underline" onclick="openIndividual('${s.id||''}')">รายละเอียด</button></div>`;
