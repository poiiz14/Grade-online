...
window.setLoadingProgress = function (pct) {
  ...
  bar.setAttribute('aria-valuenow', String(v));
  window.__loadingCurrent = v;
  updateLoadingPercent(v); // << เพิ่มบรรทัดนี้
};

// แก้ setLoadingProgressSmooth ให้เซ็ตตัวเลขด้วย
/** ตั้งเปอร์เซ็นต์แบบลื่น ๆ ไปยังเป้าหมาย (ใช้เมื่อรู้ progress จริงเป็นช่วง ๆ) — FIXED */
window.setLoadingProgressSmooth = function(targetPct, duration = 600) {
  ...
  // ใช้สถานะเริ่มจากค่าปัจจุบัน (รองรับทั้งตัวแปรเดิมและตัวแปร state)
  const start = Number((window.__loadingCurrent ?? __loadingState.current) || 0);
  ...
  function tick(now){
    ...
    // อัปเดต state ทั้งสองตัว และอัปเดตตัวเลข %
...
function isEnglishPassStatus(statusRaw){
  ...
  if (['toefl','toelf','toeic','cu tep','cutep','cu-tep','cu_tep'].some(k => norm.includes(k))) return true;
  if (/toefl\s*itp/.test(norm)) return true;
  if (/cu\s*tep/.test(norm)) return true;
  return false;
}
function isEnglishPass(test){ return isEnglishPassStatus(test && test.status); }
...
function computeEnglishPassCounts(){
  ...
  allIds.forEach(id => {
    const arr = byStu[id] || [];
    if (arr.length === 0){ neverPass++; return; } // no test => fail
...
function renderEnglishPassPie(){
  ...
  allIds.forEach(id => {
    const arr = byStu[id] || [];
    if (arr.length === 0){ fail++; return; } // no test => fail
    const ever = arr.some(t => isEnglishPass(t));
